...
export default function LLMSettingsPage() {
    ...
    const [testing, setTesting] = useState<Record<string, { status: "idle" | "loading" | "ok" | "error"; message?: string }>>({});
    ...
    const fetchConfig = async () => {
        ...
        try {
            ...
            if (data.success) {
                setProviders(data.config.llm_providers || []);
            }
        } catch (error) {
    ...
    const handleTest = async (p: LLMProvider) => {
        setTesting((prev) => ({ ...prev, [p._id || p.name]: { status: "loading" } }));
        try {
            ...
            setTesting((prev) => ({
                ...prev,
                [p._id || p.name]: { status: "error", message: error.message || "Test failed" },
            }));
        }
    };
                    ...
                    <form onSubmit={handleSave} className="space-y-6">
                        <div className="grid gap-6 sm:grid-cols-2">
                            ...
                            <div className="space-y-2">
                                ...
                                <select
                                    ...
                                    onChange={(e) =>
                                        setCurrentProvider({ ...currentProvider, type: e.target.value as any })
                                    }
                                    className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 dark:border-zinc-800 dark:bg-zinc-950"
                                >
                            ...
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    onChange={(e) =>
                                        setCurrentProvider({ ...currentProvider, model: e.target.value })
                                    }
                                    className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 dark:border-zinc-800 dark:bg-zinc-950"
                                    placeholder="e.g. deepseek-chat"
                                />
                            ...
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    onChange={(e) =>
                                        setCurrentProvider({ ...currentProvider, base_url: e.target.value })
                                    }
                                    className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 dark:border-zinc-800 dark:bg-zinc-950"
                        ...
                        <div className="space-y-2">
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    setCurrentProvider({ ...currentProvider, api_key: e.target.value })
                                }
                                className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 dark:border-zinc-800 dark:bg-zinc-950"
                                placeholder="sk-..."
                            />
                ...
                <div className="space-y-4">
                    {providers.map((p) => (
                        ...
                        >
                            <div className="flex items-center gap-4">
                                <div className={`flex h-10 w-10 items-center justify-center rounded-full ${p.type === 'chat' ? 'bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10' : 'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10'
                                    }`}>
                                    {p.type === 'chat' ? <Bot className="h-5 w-5" /> : <Database className="h-5 w-5" />}
                                </div>
                                <div>
                                    <div className="flex items-center gap-2">
                    ...
                    {providers.length === 0 && (
                        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-zinc-200 bg-zinc-50 py-12 dark:border-zinc-800 dark:bg-zinc-900/50">
                            <p className="text-zinc-500 dark:text-zinc-400">No providers configured yet.</p>
                            <button
                                onClick={startNew}
