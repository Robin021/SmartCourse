...
export default function PromptSettingsPage() {
    ...
    const fetchVersions = async (promptId: string) => {
        ...
        try {
            const res = await fetch(`/api/admin/prompts?action=versions&prompt_id=${promptId}`);
            const data = await res.json();
            if (data.success) {
    ...
    const updateVersionWeight = (version: number, weight: number) => {
        setAbConfig((prev) => ({
            ...
            versions: prev.versions.map((v) =>
                v.version === version ? { ...v, weight: Math.max(0, Math.min(100, weight)) } : v
            ),
        }));
    };
    ...
    const extractedVariables = (currentPrompt.template?.match(/\{\{(\w+)\}\}/g) || []).map((v) =>
        v.replace(/\{\{|\}\}/g, "")
    );
        ...
        <div className="mx-auto max-w-4xl">
            ...
            {isEditing ? (
                    ...
                    <form onSubmit={handleSave} className="space-y-6">
                        <div className="grid gap-6 sm:grid-cols-2">
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    onChange={(e) =>
                                        setCurrentPrompt((prev) => ({ ...prev, name: e.target.value }))
                                    }
                        ...
                        <div className="space-y-2">
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    setCurrentPrompt((prev) => ({ ...prev, description: e.target.value }))
                                }
                                className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 dark:border-zinc-800 dark:bg-zinc-950"
                        ...
                        <div className="space-y-2">
                            ...
                            <textarea
                                ...
                                onChange={(e) =>
                                    setCurrentPrompt((prev) => ({ ...prev, template: e.target.value }))
                                }
                ...
                <div className="space-y-4">
                    {prompts.map((p) => (
                        <div key={p._id} className="overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                            {/* Prompt Card Header */}
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-zinc-500 dark:text-zinc-400">
                                            {p.description || `v${p.current_version || 1} • ${p.variables.length} variable${p.variables.length !== 1 ? "s" : ""}`}
                                        </p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-2">
                                    <button
                                        ...
                                        className={`rounded-lg p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 ${showHistory === p._id
                                    ...
                                    <button
                                        ...
                                        className={`rounded-lg p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 ${showABTest === p._id
                                                ? "bg-zinc-100 text-zinc-900 dark:bg-zinc-800 dark:text-zinc-100"
                                                : "text-zinc-400"
                                            }`}
                                        title="A/B Testing"
                                                ...
                                                >
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        {v.version !== p.current_version && (
                                                            <button
                                                                onClick={() => p._id && handleRollback(p._id, v.version)}
                                ...
                                <div className="border-t border-zinc-100 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-900/50">
                                    <div className="mb-4 flex items-center justify-between">
                                        ...
                                        <label className="flex items-center gap-2">
                                            <span className="text-xs text-zinc-500">Enable A/B Test</span>
                                            <button
                                                onClick={() =>
                                                    setAbConfig((prev) => ({ ...prev, enabled: !prev.enabled }))
                                                }
                                                className={`relative h-5 w-9 rounded-full transition-colors ${abConfig.enabled ? "bg-indigo-600" : "bg-zinc-300 dark:bg-zinc-600"
                                                    }`}
                                            >
                                                <span
                                                    className={`absolute left-0.5 top-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform ${abConfig.enabled ? "translate-x-4" : ""
                                                        }`}
                                                />
                                            </button>
                                        </label>
                                    ...
                                    {abConfig.enabled && (
                                        <>
                                            ...
                                            <div className="mb-4">
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {Array.from({ length: p.current_version || 1 }, (_, i) => i + 1).map(
                                                        (v) => {
                                                            const inTest = abConfig.versions.some((ver) => ver.version === v);
                                                            return (
                                                                <button
                                                                    key={v}
                                                                    onClick={() => (inTest ? removeVersionFromAB(v) : addVersionToAB(v))}
                                                                    className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold transition-colors ${inTest
                                                ...
                                                <div className="mb-4 space-y-3">
                                                    ...
                                                    {abConfig.versions.map((v) => (
                                                        <div key={v.version} className="flex items-center gap-3">
                                                            ...
                                                            <input
                                                                ...
                                                                value={v.weight}
                                                                onChange={(e) =>
                                                                    updateVersionWeight(v.version, parseInt(e.target.value))
                                                                }
                                                    ...
                                                    {abConfig.versions.length >= 2 && (
                                                        <p className="text-xs text-amber-600 dark:text-amber-400">
                                                            ...
                                                            {abConfig.versions.reduce((sum, v) => sum + v.weight, 0)}%
                                                        </p>
                                                    )}
                                                </div>
                                            )}
                    ...
                    {prompts.length === 0 && (
                        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-zinc-200 bg-zinc-50 py-12 dark:border-zinc-800 dark:bg-zinc-900/50">
                            <p className="text-zinc-500 dark:text-zinc-400">No prompts configured.</p>
                            <button
                                onClick={startNew}
